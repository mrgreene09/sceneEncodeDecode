...
load order;
load humanDist; %finalFeaturesZ; %allFeatures;
%load humanDist;
%allFeatures = cat(2,ones(435,1),allFeaturesEuclidean);
%load allCNNfeatures; % pca features<t_k>
allFeatures = humanDist; %attributesWhite; %cat(1, ones(size(humanDist)), humanDist);
...
for s = 1:numSub
   ...
   % dataMat = normMat;
    ...
    save -v7.3 rSquare500behavior regressionRsquare;
